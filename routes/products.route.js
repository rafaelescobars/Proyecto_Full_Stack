const jwt = require("jsonwebtoken");
const { Router } = require("express");

const {
  productsGetQuery,
  productPostQuery,
  productGetQuery,
  productPutQuery,
  productDeleteQuery,
} = require("../queries/productsQueries.js");
const { ownersGetQuery } = require("../queries/ownersQueries.js");

const secretKey = process.env.SECRET_KEY;

const router = Router();

//middleware /
router.use("/", (req, res, next) => {
  try {
    req.loggedUSer = jwt.verify(req.cookies.token, secretKey).data;
    next();
  } catch (err) {
    res.redirect("/login");
  }
});

//Route users GET
router.get("/", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    productsGetQuery().then((value) => {
      res.render("products", {
        userType: req.loggedUSer.admin,
        products: value,
        userId: req.loggedUSer.id,
        userEmail: req.loggedUSer.email,
      });
    });
  } else {
    res.redirect("/login");
  }
});

//Route newUser GET
router.get("/new", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    ownersGetQuery().then(
      (value) => {
        res.render("newProduct", {
          userType: req.loggedUSer.admin,
          owners: value,
          userId: req.loggedUSer.id,
          userEmail: req.loggedUSer.email,
        });
      },
      (reason) => {
        console.log(reason);
      }
    );
  } else {
    res.redirect("/login");
  }
});

//Route newUser POST
router.post("/new", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    productPostQuery(Object.values(req.body)).then(
      (value) => {
        res.sendStatus("200");
      },
      (reason) => {
        console.log(reason);
      }
    );
  }
});

//Route editUSer GET
router.get("/edit/:id", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    const { id } = req.params;
    productGetQuery([id]).then(
      (productValue) => {
        ownersGetQuery().then(
          (ownersValue) => {
            res.render("editProduct", {
              product: productValue,
              owners: ownersValue,
              userType: req.loggedUSer.admin,
              userId: req.loggedUSer.id,
              userEmail: req.loggedUSer.email,
            });
          },
          (reason) => {
            console.log(reason);
          }
        );
      },
      (reason) => {
        console.log(reason);
      }
    );
  } else {
    res.redirect("/login");
  }
});

//Route editUser PUT
router.put("/edit", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    productPutQuery(Object.values(req.body)).then(
      (value) => {
        res.sendStatus("200");
      },
      (reason) => {
        console.log(reason);
      }
    );
  }
});

//Route deleteUser DELETE
router.delete("/delete/:id", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    const { id } = req.params;
    productDeleteQuery([id]).then(
      (value) => {
        // res.redirect('/')
        res.sendStatus(200);
      },
      (reason) => {
        console.log(reason);
      }
    );
  }
});

//Route editUSer GET
router.get("/get/:productId", (req, res) => {
  if (req.loggedUSer != undefined && req.loggedUSer.admin == true) {
    const { productId } = req.params;
    productGetQuery([productId]).then(
      (value) => {
        res.send(value);
      },
      (reason) => {
        console.log(reason);
      }
    );
  }
});

module.exports = router;
